...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    val leftmost = try { left.mostRetweeted } catch { case _: Throwable => elem }
    val rightmost  = try { right.mostRetweeted } catch { case _: Throwable => elem }

    if (elem.retweets > leftmost.retweets &&
        elem.retweets > rightmost.retweets)
      elem
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet =
    TweetReader.allTweets.filter(t => google.exists(gw => t.text.contains(gw)))
  lazy val appleTweets: TweetSet =
    TweetReader.allTweets.filter(t => apple.exists(aw => t.text.contains(aw)))

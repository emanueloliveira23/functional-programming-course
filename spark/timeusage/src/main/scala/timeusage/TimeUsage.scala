...
//    val finalDf = timeUsageGroupedTyped(summaryTypedDf)
    finalDf.show()
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def isPrimaryNeeds(columnName: String) =
    ...
    def isWork(columnName: String) =
      columnName.startsWith("t05") || columnName.startsWith("t1805")

    def isOther(columnName: String) =
    ...
    columnNames
      .foldLeft((List.empty[Column], List.empty[Column], List.empty[Column])) {
        case ((primaryNeeds, work, other), columnName) =>
          if (isPrimaryNeeds(columnName))
            (col(columnName) :: primaryNeeds, work, other)
          else if (isWork(columnName))
            (primaryNeeds, col(columnName) :: work, other)
          else if (isOther(columnName))
            (primaryNeeds, work, col(columnName) :: other)
          else
